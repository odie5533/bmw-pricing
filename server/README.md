...
- 🎯 Entry points to execute your program using `python -m <bmw_backend>` or `$ bmw_backend` with basic CLI argument parsing.
...
```bash
git clone https://github.com/author_name/project_urlname bmw_backend
cd bmw_backend
make install
```
...
```bash
bmw_backend create-db   # run once
bmw_backend populate-db  # run once (optional)
bmw_backend add-user -u admin -p 1234  # ads a user
bmw_backend run
```
